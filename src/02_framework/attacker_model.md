...
Our attacker model is in most parts based on the original attacker model of the OWASP ISTG. We have adopted their conceptual basis and their access levels unmodified. To assess the severity of a security breach and/or the sensitivity of the data found on an IoT device we additionally introduced security levels (inspired from the security levels introduced by OWASP IVSV). These security levels should help with selecting appropriate test cases and with assessing the results of said test cases. 
...
[^5]: There are devices where internal interfaces can also be accessed from outside (e.g. UART interface on Dreame L10 Pro). See [device model](./device_model### Interfaces) for further information.

[^6]: Physical interfaces might be testable with local physical access (*PA-2*) under certain circumstances, e.g., if the physical interface is connected to a local network.

[^7]: User interfaces might be testable with local (*PA-2*) or remote physical access (*PA-1*), depending on if they were designed for that kind of access, e.g., for remote control or monitoring purposes.



## Security Levels

Different kind of IoT devices require different levels of security. A security breach of a simple light bulb has less severe consequences as a security breach of a critical life-support system. Ideally all IoT devices should adhere to the highest possible security level, but in reality a compromise needs to me made between security, costs or convenience. Also, sometimes, what could be considered a security breach can also be seen as a feature. E.g. the lack of verification of a firmware update can be seen as an attack vector but also also as a way for makers to install their own custom firmwares.

We have identified four different dimensions that allow to measure the amount of protection a specific device requires and the impact of a sucessful security breach:

- **Physical Access Security:** Device should be protected against attacks on which maximal physical access level. Already covered in the access level section above.
- **Authorization Access Security:** Device should be protected against attacks on which maximal authorization access level. Already covered in the access level section above.
- **Data Security:** Describes how sensitive is the data that is stored on the device and which amount of protection does it require. This dimension also covers legal requirements imposed by data protection laws.
- **Security Impact:** Describes the severity of the impact of a sucessful security breach. E.g. when a simple light bulb is breached then the attackers may only be able to control lighting, but when a critical life-support system is breached then human lifes may be in danger.

### Data Security Levels

The following data security levels are defined:

1.  **Non-Critical Data (*DS-1*):** There is no sensitive or privacy relevant data stored on the device.

2.  **Sensitive Data (*DS-2*):** There is sensitive data stored on the device. E.g. passwords, ...

3.  **Privacy Related Data (*DS-3*):** There privacy related data stored on the device. E.g. personal data, data subject to data protection laws, ...

4.  **Confidential Data (*DS-4*):** There is confidential data stored on the device that should be protected at all costs. E.g. trade secrets, military secrets, ...

### Security Impact Levels

The following security impact levels are defined:

1.  **Insignificant Impact (*SI-1*):** A security breach has no significant or only very minor impact.

2.  **Minor Impact (*SI-2*):** A security breach has minor impact.

3.  **Major Impact (*SI-3*):** A security breach has major impact. E.g. smart lock

4.  **Critical Impact (*SI-4*):** A security breach has critical impact and should therefore be avoided at all costs.
